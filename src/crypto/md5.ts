/**
 * Pure TypeScript MD5 implementation for cross-platform ArrayBuffer hashing.
 * RFC 1321 compliant, works on Desktop and Mobile (iOS / Android).
 */
...
export function md5(buffer: ArrayBuffer | Uint8Array): string {
  ...
  words[nWords - 2] = (nBytes * 8) & 0xffffffff;
  words[nWords - 1] = Math.floor((nBytes * 8) / 0x100000000);
  ...
  for (let i = 0; i < 4; i++) {
    for (let j = 0; j < 4; j++) {
      const byte = (arr[i] >>> (j * 8)) & 0xff;
      result += hexDigits.charAt((byte >>> 4) & 0x0f) + hexDigits.charAt(byte & 0x0f);
    }
  }
